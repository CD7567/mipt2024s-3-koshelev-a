...
void testString(std::ofstream& out, const size_t test_max_size) {
    ...
    for (size_t i = 0; i < test_max_size; ++i) {
        size_t copy_construction_time = timeit<std::chrono::nanoseconds>(
            [&str] { std::string copy_constructed(str); });

        out << dyn_format(PRINT_FORMAT, "COPY", i, "CONSTRUCTION",
                          copy_construction_time)
            << "\n";

        size_t copy_assignment_time = timeit<std::chrono::nanoseconds>([&str] {
            std::string copy_assigned;
            copy_assigned = str;
        });
        ...
        size_t move_construction_time = timeit<std::chrono::nanoseconds>(
            [&tmp] { std::string move_constructed(std::move(tmp)); });
        ...
        size_t move_assignment_time = timeit<std::chrono::nanoseconds>([&tmp] {
            std::string move_assigned;
